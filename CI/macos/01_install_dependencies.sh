...
install_qt-deps() {
    ...
    #PRISM/renjinbo/20230207/#/prism ignore obs qt
    status "prism not need obs qt, so ignore download"
    return
    ensure_dir "${DEPS_BUILD_DIR}"
    step "Download..."
...
install_cef() {
    ...
    if [ -z "${_SKIP}" ]; then
        ...
        cmake ${CCACHE_OPTIONS} ${QUIET:+-Wno-deprecated -Wno-dev --log-level=ERROR} \
            ...
            -DCMAKE_CXX_FLAGS="-std=c++11 -stdlib=libc++ -Wno-deprecated-declarations -Wno-unknown-warning-option -Wno-deprecated-builtins" \
