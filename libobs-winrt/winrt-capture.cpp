...
struct winrt_capture {
	...
	void on_frame_arrived(winrt::Windows::Graphics::Capture::Direct3D11CaptureFramePool const &sender,
			      winrt::Windows::Foundation::IInspectable const &)
	{
		//PRISM/wangshaohui/20250508/NELO-300/check status of device ------------ start
		if (context) {
		...
		} //PRISM/wangshaohui/20250508/NELO-300/check status of device ------------ end
		...
		if (desc.Format == get_pixel_format(window, monitor, force_sdr)) {
			//PRISM/WangShaohui/20240201/#4181/Use wgc for region capture - check {is_desktop}
			if (is_desktop || !client_area ||
				...
				if (is_desktop) {
					...
					client_box.bottom = texture_region.bottom;
					client_box.front = 0;
					client_box.back = 1;
				}
			...
			bool valid_size = (frame_content_size.Width > 0 && frame_content_size.Height > 0);
...
try {
	...
	auto result = winrt::Windows::Foundation::Metadata::ApiInformation::IsPropertyPresent(
		L"Windows.Graphics.Capture.GraphicsCaptureSession", L"IsBorderRequired");
	blog(LOG_INFO, "winrt_capture_border_toggle_supported: %s", result ? "true" : "false");
	return result;
...
{
	try { //PRISM/wangshaohui/20230719/#1981/catch exception of WGC
		winrt_capture *capture = static_cast<winrt_capture *>(data);
		...
		const winrt::Windows::Graphics::Capture::Direct3D11CaptureFramePool frame_pool =
			winrt::Windows::Graphics::Capture::Direct3D11CaptureFramePool::Create(
				device,
				static_cast<winrt::Windows::Graphics::DirectX::DirectXPixelFormat>(capture->format), 2,
				capture->last_size);
		...
		//PRISM/wangshaohui/20230719/#1981/catch exception of WGC
		//try {
...
//PRISM/WangShaohui/20240201/#4181/Use wgc for region capture
extern "C" EXPORT struct winrt_capture *winrt_capture_init_desktop(BOOL cursor, HMONITOR monitor, BOOL force_sdr,
								   RECT tex_region)
{
	...
	auto cap = winrt_capture_init_internal(cursor, NULL, FALSE, force_sdr, monitor);
	if (cap) {
		cap->is_desktop = TRUE;
...
extern "C" EXPORT void winrt_capture_on_display_changed()
{
	...
	try {
		...
		blog(LOG_ERROR, "winrt_capture_display_changed (0x%08X): %s", err.code().value,
		     winrt::to_string(err.message()).c_str());
	} catch (...) {
		blog(LOG_ERROR, "winrt_capture_display_changed (0x%08X)", winrt::to_hresult().value);
	}
}
