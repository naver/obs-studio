...
bool WHIPOutput::Start()
{
	...
	if (!obs_output_initialize_encoders(output, 0)) {
		//PRISM/wangshaohui/20241120/none/add logs
		blog(LOG_INFO, "%p-%s failed to init encoder", output, __FUNCTION__);
		return false;
	}
...
void WHIPOutput::Data(struct encoder_packet *packet)
{
	...
	if (audio_track && packet->type == OBS_ENCODER_AUDIO) {
		...
		//PRISM/wangshaohui/20250311/2446/for network time
		pls_output_on_video_sent(output, os_gettime_ns() - start_time);
	}
}
...
static std::string getRtcLogLevelStr(rtcLogLevel level)
{
	std::string str = "";
	switch (level) {
	case rtcLogLevel::RTC_LOG_NONE:
		str = "none";
		break;
	case rtcLogLevel::RTC_LOG_FATAL:
		str = "fatal";
		break;
	...
	case rtcLogLevel::RTC_LOG_DEBUG:
		str = "debug";
		break;
	case rtcLogLevel::RTC_LOG_VERBOSE:
		str = "verbose";
		break;
	default:
		break;
...
//PRISM/FanZirong//20241206/PRISM_PC-1715/print libdatachannel log
void RTC_API WHIPOutput::OnRtcLog(rtcLogLevel level, const char *message)
{
	blog(LOG_INFO, "[obs-webrtc][rtc][%s] %s", getRtcLogLevelStr(level).c_str(), message);
}
...
{
	//PRISM/FanZirong//20241206/PRISM_PC-1715/print libdatachannel log
	rtcInitLogger(rtcLogLevel::RTC_LOG_FATAL, OnRtcLog);
	rtcInitLogger(rtcLogLevel::RTC_LOG_ERROR, OnRtcLog);
	rtcInitLogger(rtcLogLevel::RTC_LOG_WARNING, OnRtcLog);
	rtcInitLogger(rtcLogLevel::RTC_LOG_INFO, OnRtcLog);
	rtcInitLogger(rtcLogLevel::RTC_LOG_DEBUG, OnRtcLog);
	//PRISM/FanZirong//20241206/PRISM_PC-1715/print libdatachannel log
	...
	peer_connection->onStateChange([this](rtc::PeerConnection::State state) {
		...
		case rtc::PeerConnection::State::Failed:
			//PRISM/FanZirong/20241203/PRISM_PC-1675/add log
			do_logex(false, LOG_INFO, fields, 1, "PeerConnection state is now: Failed");
			Stop(false);
			obs_output_signal_stop(output, OBS_OUTPUT_ERROR);
			break;
...
{
	//PRISM/WuLongyue/20231212/#2212
	std::shared_ptr<int> _(nullptr, [](int *) { obs_video_dec_active(); });
...
void WHIPOutput::StopThread(bool signal)
{
	...
	//PRISM/FanZirong//20241206/PRISM_PC-1715/print libdatachannel log
	rtcInitLogger(rtcLogLevel::RTC_LOG_FATAL, NULL);
	rtcInitLogger(rtcLogLevel::RTC_LOG_ERROR, NULL);
	rtcInitLogger(rtcLogLevel::RTC_LOG_WARNING, NULL);
	rtcInitLogger(rtcLogLevel::RTC_LOG_INFO, NULL);
