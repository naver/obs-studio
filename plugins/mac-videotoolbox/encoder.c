...
#define VT_BLOG_EVENT(level, format, ...)                                                                       \
	const char *fields[][2] = {{PTS_LOG_TYPE, PTS_TYPE_EVENT}};                                             \
	blogex(false, level, fields, 1, "[VideoToolbox %s: '%s']: " format, obs_encoder_get_name(enc->encoder), \
...
{
	//PRISM/cao.kewei/20231115/#2745/add log
	VT_LOG(LOG_INFO,
	       "session_set_bitrate:\n"
	       "\trate_control:          %s\n"
	       "\tnew_bitrate:            %d (kbps)\n"
	       ...
	       "\tlimit_bitrate:          %s\n"
	       "\tmax_bitrate:            %d (kbps)\n"
	       "\tmax_bitrate_window:     %f\n",
	       rate_control, new_bitrate, quality, limit_bitrate ? "true" : "false", max_bitrate, max_bitrate_window);
	//PRISM/cao.kewei/20231115/#2745/add log end
...
static bool vt_encode(void *data, struct encoder_frame *frame, struct encoder_packet *packet, bool *received_packet)
{
	...
	CFMutableDictionaryRef frameProperties =
		CFDictionaryCreateMutable(NULL, 1, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);

	if (obs_encoder_request_keyframe_and_clean(enc->encoder)) {
	...
	code = VTCompressionSessionEncodeFrame(enc->session, pixbuf, pts, dur, frameProperties, pixbuf, NULL);
...
void obs_module_post_load(void)
{
	...
	for (CFIndex i = 0; i < size; i++) {
		...
		if (strcmp(id, "com.apple.videotoolbox.videoencoder.h264") &&
		    strcmp(id, "com.apple.videotoolbox.videoencoder.hevc.vcp")) {
