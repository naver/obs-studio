...
class OBSCameraDeviceSource: NSObject, CMIOExtensionDeviceSource {
    ...
    init(localizedName: String, deviceUUID: UUID, sourceUUID: UUID, sinkUUID: UUID) {
        ...
        _streamSource = OBSCameraStreamSource(
            localizedName: "PRISM Camera Extension Stream Source", //PRISM/Zhongling/20231123/#/VCAM
            streamID: sourceUUID,
        ...
        _streamSink = OBSCameraStreamSink(
            localizedName: "PRISM Camera Extension Stream Sink", //PRISM/Zhongling/20231123/#/VCAM
            streamID: sinkUUID,
            streamFormat: videoStreamFormat,
            device: device
        )
        ...
        -> CMIOExtensionDeviceProperties
    {
        ...
        if properties.contains(.deviceModel) {
            deviceProperties.model = "PRISM Camera Extension" //PRISM/Zhongling/20231123/#/VCAM
        }
    ...
    func startStreaming() {
        ...
        _placeholderTimer!.schedule(
            deadline: .now(),
			repeating: 1.0, //1.0 / Double(OBSCameraFrameRate), //PRISM/Zhongling/20231128/#/Lower CPU consume
            leeway: .seconds(0)
        )
