...
static void update_settings(struct window_capture *wc, obs_data_t *s)
{
	...
	if (wc->title != NULL) {
		blog(LOG_INFO,
		     ...
		     "\t priority: %d \n"
		     "\t method: %d",
		     obs_source_get_name(wc->source), wc->source, wc, wc->executable, wc->class ? wc->class : "",
...
static obs_properties_t *wc_properties(void *data)
{
	...
	//PRISM/Xiewei/20240308/#4063/fix a crash caused by executing a null pointer function
	obs_property_list_item_disable(p, 2, !wgc_supported || (wc != NULL && wc->exports.winrt_capture_supported == NULL));
...
static void wc_tick(void *data, float seconds)
{
	...
	if (!wc->previously_visible) {
		...
		blog(LOG_INFO, "[window-capture: '%s'] [obs_source:%p plugin_obj:%p] '%s' is visible now.",
		     obs_source_get_name(wc->source), wc->source, wc, wc->executable);
	}
