...
//PRISM/Xiewei/20240710/#5555/Fix a crash where access the invalid winrt exported functions
static inline void update_settings(struct duplicator_capture *capture, obs_data_t *settings, bool wgc_init)
{
	...
	//PRISM/Xiewei/20240710/#5555/Fix a crash where access the invalid winrt exported functions
	capture->method =
		choose_method((int)obs_data_get_int(settings, "method"), wgc_supported && wgc_init, monitor.handle);
...
static void *duplicator_capture_create(obs_data_t *settings, obs_source_t *source)
{
	...
	if (graphics_uses_d3d11) {
		...
		if (capture->winrt_module) {
			//PRISM/Xiewei/20240710/#5555/Fix a crash where access the invalid winrt exported functions
			wgc_init = load_winrt_imports(&capture->exports, capture->winrt_module, module);
		}
	}

	//PRISM/Xiewei/20240710/#5555/Fix a crash where access the invalid winrt exported functions
	capture->wgc_init = wgc_init;
	//PRISM/Xiewei/20240710/#5555/Fix a crash where access the invalid winrt exported functions
	update_settings(capture, settings, wgc_init);
...
static void update_settings_visibility(obs_properties_t *props, struct duplicator_capture *capture)
{
	...
	//PRISM/Xiewei/20240702/#5745/fix a crash caused by executing a null pointer function
	const bool wgc_cursor_toggle = wgc_options && capture->capture_winrt &&
...
static bool display_capture_method_changed(obs_properties_t *props, obs_property_t *p, obs_data_t *settings)
{
	...
	//PRISM/Xiewei/20240710/#5555/Fix a crash where access the invalid winrt exported functions
	update_settings(capture, settings, capture->wgc_init);
