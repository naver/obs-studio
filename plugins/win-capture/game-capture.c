...
#define do_logex(kr, level, fields, field_count, format, ...)                          \
	{                                                                              \
		blogex(kr, level, fields, field_count, "[game-capture: '%s'] " format, \
...
static void game_capture_update(void *data, obs_data_t *settings)
{
	...
	do_logex(false, LOG_INFO, fields, 2,
		 ...
		 "\tcapture_overlays: %d\n"
		 "\thook_rate: %d",
		 cfg.title ? cfg.title : "empty", cfg.class ? cfg.class : "empty", cfg.executable, cfg.mode,
		 cfg.priority, cfg.force_shmem, cfg.anticheat_hook, cfg.allow_transparency, cfg.limit_framerate,
...
static void pipe_log(void *param, uint8_t *data, size_t size)
{
	...
	if (data && size) {
		...
		if ((uint64_t)sub == (uint64_t)data && size > key_len) {
			...
			pls_on_game_render_type(gc->game_title, type);
		}
	}
}
...
static inline bool init_pipe(struct game_capture *gc)
{
	...
	if (!ipc_pipe_server_start(&gc->pipe, name, pipe_log, gc)) {
		...
		if (gc->init_pipe_failed_num == 0 ||
		    (gc->init_pipe_failed_num >= 15 && gc->init_pipe_failed_num % 15 == 0)) {
			warn("init_pipe: failed to start pipe %lu", GetLastError());
		}
...
static void log_game_capture_failed(struct game_capture *gc, enum obs_source_event_type type, const char *reason)
{
	...
	HANDLE process = open_process(PROCESS_QUERY_INFORMATION, false, game_process_id);
	if (!process)
		goto done;
	...
	snprintf(exe_version, sizeof(exe_version), "%d.%d.%d.%d", ver_info.major, ver_info.minor, ver_info.build,
		 ver_info.revis);
...
static void setup_window(struct game_capture *gc, HWND window)
{
	...
	if (gc->wait_for_target_startup) {
		...
		if (gc->next_window != window) {
			...
			char temp[256] = {0};
			pls_extract_file_name(exe.array, temp, (sizeof(temp) / sizeof(temp[0])) - 1);

			snprintf(gc->game_title, MAX_PATH, "%s", title.array ? title.array : temp);
