...
static void add_folder(struct slideshow *ss, os_dir_t *dir, const char *path, image_file_array_t *new_files,
			...
			for (;;) {
				...
				if (ss->mem_usage >= MAX_MEM_USAGE)
					break;
                        ...
                        os_closedir(dir);
			*/
			add_folder(ss, dir, path, &new_files, &cx, &cy);
...
static obs_properties_t *ss_properties(void *data)
{
	...
	for (size_t i = 0; i < contexts; i++) {
		if (obs_get_video_info_by_index(i, &ovi)) {
			snprintf(str, 32, "%dx%d", ovi.base_width, ovi.base_height);
			obs_property_list_add_string(p, str, str);
		}
	}
