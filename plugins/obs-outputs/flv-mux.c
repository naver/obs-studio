				...
				size_t *size)
{
	...
	*enc++ = AMF_ECMA_ARRAY;
	...
	bool is_hevc = (codec_id && (0 == strcmp(codec_id, "hevc")));
	if (is_hevc) {
	...
	//PRISM/WuLongyue/20230725/#None/change publisher info
	dstr_printf(&encoder_name, "PRISM %s(libobs version ", OBS_VERSION);
...
static void flv_video(struct serializer *s, int32_t dts_offset,
		      struct encoder_packet *packet, bool is_header, bool is_hevc)
{
	...
	//PRISM/WuLongyue/20230726/None/support HEVC
	if (is_hevc) {
		s_w8(s, packet->keyframe ? 0x1c : 0x2c);
	} else {
		    ...
		    bool is_hevc)
{
	...
	if (packet->type == OBS_ENCODER_VIDEO)
		flv_video(&s, dts_offset, packet, is_header, is_hevc);
	else
