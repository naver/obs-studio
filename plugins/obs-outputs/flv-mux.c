...
{
	//PRISM/Xiewei/20240910/#6104/crash when access destroyed encoders
	if (!encoder)
		return 0.0;
				...
				size_t *size, bool is_naver_hevc)
{
	...
	*enc++ = AMF_ECMA_ARRAY;
	...
	const char *pls_version = pls_get_version_string();
	if (NULL == pls_version || 0 == pls_version[0]) {
		assert(false);
	}
		    ...
		    bool is_naver_hevc)
{
	...
	if (packet->type == OBS_ENCODER_VIDEO)
		flv_video(&s, dts_offset, packet, is_header, is_naver_hevc);
	else
