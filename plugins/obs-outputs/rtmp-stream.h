...
#define do_log_ex(level, fields, count, format, ...)                                                            \
	blogex(false, level, fields, count, "[rtmp stream: '%s'] " format, obs_output_get_name(stream->output), \
	       ##__VA_ARGS__)

#define warn_ex(fields, count, format, ...) do_log_ex(LOG_WARNING, fields, count, format, ##__VA_ARGS__)
#define info_ex(fields, count, format, ...) do_log_ex(LOG_INFO, fields, count, format, ##__VA_ARGS__)
...
struct rtmp_stream {
	...
	//PRISM/WuLongyue/20240116/#4031/May not detach thread (true: connect_thread is being joining)
