...
void RenderContext::UpdateFont()
{
	...
	if (custom_font && !face.empty()) {
		...
		char faceName[512] = {0};
		if (lf.lfFaceName) {
			size_t len = wcslen(lf.lfFaceName);
			os_wcs_to_utf8(lf.lfFaceName, len, faceName, 512);
		}
		...
		if (!pls_design_mode()) {
			...
			swprintf_s(
				tip,
				L"[Design-mode] Failed to load font: '%ls', please ensure that you installed it into system.",
				lf.lfFaceName);
			MessageBox(nullptr, tip, L"Text font error", MB_ICONWARNING);
		}
	}
	...
	if (hfont) {
		...
		if (font->GetLastStatus() != Ok) {
			...
			blog(LOG_INFO, "Failed to load font: '%s' by GDI+, find it in private collection. result: %d",
			     faceName, result);
		} else {
	...
	if (!obs_obj_is_private(source)) {
		...
		if (json && *json) {
			blog(LOG_INFO,
			     ...
			     "%s",
			     obs_source_get_name(source), (void *)source, (void *)this, json);
		}
	}
	//PRISM/Xiewei/20240325/#4640/add log
