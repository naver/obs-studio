	...
	IDeckLinkAudioInputPacket *audioPacket, const uint64_t timestamp)
{
	...
	//PRISM/Fanzirong/20240407/noissue/add log
	if (is_first_audio) {
		is_first_audio = false;
		LOG(LOG_INFO, "DeckLink: first audio received, frames %u, pts %llu",
			currentPacket.frames, timestamp);
	}
	...
	IDeckLinkVideoInputFrame *videoFrame, const uint64_t timestamp)
{
	...
	if (is_first_video) {
		...
		LOG(LOG_INFO, "DeckLink: first video received, width %u, height %u, size %u, pts %llu",
			(uint32_t)frame->GetWidth(), (uint32_t)frame->GetHeight(),
			(uint32_t)frame->GetRowBytes(), timestamp);
	}
