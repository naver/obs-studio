...
static void amf_encode_base(amf_base *enc, AMFSurface *amf_surf, encoder_packet *packet, bool *received_packet)
{
	...
	*received_packet = false;
	...
	while (waiting) {
		...
		wchar_t *force_key_name = (wchar_t *)AMF_VIDEO_ENCODER_FORCE_PICTURE_TYPE;
		amf_int64 force_value_name = AMF_VIDEO_ENCODER_PICTURE_TYPE_IDR;

		switch (enc->codec) {
		case amf_codec_type::AV1: {
			force_key_name = (wchar_t *)AMF_VIDEO_ENCODER_AV1_FORCE_FRAME_TYPE;
			force_value_name = AMF_VIDEO_ENCODER_AV1_FORCE_FRAME_TYPE_KEY;
			break;
		}
		case amf_codec_type::HEVC: {
			force_key_name = (wchar_t *)AMF_VIDEO_ENCODER_HEVC_FORCE_PICTURE_TYPE;
			force_value_name = AMF_VIDEO_ENCODER_HEVC_PICTURE_TYPE_IDR;
			break;
		}
		default:
			break;
