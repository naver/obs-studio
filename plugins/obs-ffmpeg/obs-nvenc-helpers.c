		...
		const char *func, const char *call)
{
	...
	default:
		...
		//PRISM/WuLongyue/20231205/#3331/No function name
		error("%s", error_message.array);
		//obs_encoder_set_last_error(encoder, error_message.array);
		dstr_free(&error_message);
		break;
	...
	if (nvenc_error && *nvenc_error) {
		...
		blogex(false, LOG_ERROR, fields, 1, "%s: %s failed: %d (%s): %s", func, call, (int)err,
		      nv_error_name(err), nvenc_error);
	} else {
		//PRISM/chenguoxi/20241212/PRISM_PC-1672/Add logs
		//error("%s: %s failed: %d (%s)", func, call, (int)err,
		//      nv_error_name(err));
		blogex(false, LOG_ERROR, fields, 1, "%s: %s failed: %d (%s)", func, call, (int)err,
		      nv_error_name(err));
	}
	return true;
}
