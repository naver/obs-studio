...
static void *reconnect_thread(void *param)
{
	...
	//PRISM/Keven/20250527/PRISM_PC_NELO-319/crash fix
	if (!ca || !ca->exit_event) {
		return NULL;
	}
	...
	//PRISM/Keven/20250527/PRISM_PC_NELO-319/crash fix
	while (ca->exit_event && os_event_timedwait(ca->exit_event, ca->retry_time) == ETIMEDOUT) {
...
static void coreaudio_begin_reconnect(struct coreaudio_data *ca)
{
	...
	//PRISM/Keven/20250527/PRISM_PC_NELO-319/crash fix
	if (!ca || !ca->exit_event || ca->reconnecting)
		return;
...
static void coreaudio_shutdown(struct coreaudio_data *ca)
{
	...
	//PRISM/cao.kewei/20240312/#4649/coreaudio crash
	//	if (ca->reconnecting) {
	//PRISM/Keven/20250527/PRISM_PC_NELO-319/crash fix
	if (ca->exit_event) {
		os_event_signal(ca->exit_event);
	}
	if (ca->reconnect_thread) {
		pthread_join(ca->reconnect_thread, NULL);
		ca->reconnect_thread = NULL;
	}
	//PRISM/Keven/20250527/PRISM_PC_NELO-319/crash fix
	if (ca->exit_event) {
