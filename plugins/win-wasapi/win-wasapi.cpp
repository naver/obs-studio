...
void WASAPISource::LogSettings()
{
	...
	if (sourceType == SourceType::ProcessOutput) {
		...
		blogex(false, LOG_INFO, fields, 2,
		       ...
		       "\tclass: %s\n"
		       "\tpriority: %d",
		       obs_source_get_name(source), source, executable.c_str(), title.c_str(), window_class.c_str(),
		       (int)priority);
	} else {
		//PRISM/FanZirong/20241203/PRISM_PC-1675/add log fields
		blogex(false, LOG_INFO, fields, 2,
		       ...
		       "\tdevice id: %s\n"
		       "\tuse device timing: %d",
		       obs_source_get_name(source), source, device_id.c_str(), (int)useDeviceTiming);
	}
}
...
void WASAPISource::Initialize()
{
	...
	blog(LOG_INFO, "WASAPI: Device '%s' [%" PRIu32 " Hz] [%u channels] initialized (source: %s)",
	     device_name.c_str(), sampleRate, get_audio_channels(speakers), obs_source_get_name(source));
...
bool WASAPISource::ProcessCaptureData()
{
	...
	while (true) {
		...
		//PRISM/FanZirong/20240222/4482/buffer is NULL
		if (!buffer) {
			blog(LOG_WARNING,
			     "[WASAPISource::ProcessCaptureData]"
			     "getbuffer is NULL, frames = %u",
			     frames);
...
static void *CreateWASAPISource(obs_data_t *settings, obs_source_t *source, SourceType type)
{
	try {
		...
		//PRISM/FanZirong/20240625/5688/Catch HRError to avoid crash
	} catch (HRError &error) {
		blog(LOG_ERROR, "[CreateWASAPISource]: %s: %lX", error.str, error.hr);
	}
