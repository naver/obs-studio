						...
						uint32_t last_obs_ver)
{
	...
	blog(LOG_INFO, "%ssource '%s' (id: %s) created source: %p plugin:%p", private ? "private " : "", name, id,
	     source, source->context.data);
...
void obs_source_destroy(struct obs_source *source)
{
	...
	//PRISM/FanZirong/20231227/noissue/add log
	uint64_t sample_rate = audio_output_get_sample_rate(obs->audio.audio);
	uint64_t ignore_duration = conv_frames_to_time(sample_rate, source->ignore_audio_frames);
	...
	//PRISM/wangshaohui/20230720/none/add logs
	blog(LOG_INFO, "obs_source_destroy started. source:%p plugin:%p id:%s name:'%s'", source, source->context.data,
...
static void obs_source_destroy_defer(struct obs_source *source)
{
	...
	//PRISM/wangshaohui/20230720/none/add logs
	blog(LOG_INFO, "%p id=%s to destroy source", source, source->info.id);
	...
	blogex(false, LOG_INFO, fields, 3,
	       ...
	       source->context.private ? "private " : "", source->context.name, source, source->context.data,
	       source->info.id);
	...
	//PRISM/wangshaohui/20230720/none/add logs
	uint64_t token_time = (os_gettime_ns() - start_time) / 1000000;
	blog(LOG_INFO, "obs_source_destroy_defer completed. source:%p token_time:%llu ms", source, token_time);
}
...
void obs_source_release(obs_source_t *source)
{
	...
	if (!pls_is_alive(source)) {
		blog(LOG_WARNING, "you are releasing destroied source: %p", source);
		assert(false && "you are releasing destroied source");
		return;
	}
...
obs_source_t *obs_source_get_ref(obs_source_t *source)
{
	...
	//PRISM/wangshaohui/20230724/#2093/check source alive
	if (!pls_is_alive(source)) {
		blog(LOG_INFO, "source has been destroied : %p", source);
		return NULL;
	}
...
void obs_source_set_audio_has_cover(obs_source_t *source, bool has_cover, bool is_ready)
{
	if (source) {
		source->is_audio_has_cover = has_cover;
		source->is_audio_has_cover_ready = is_ready;
	}
}
...
{
	//PRISM/FanZirong/20231227/noissue/add log
	source->reset_audio_frames += source->audio_input_buf[0].size / sizeof(float);
...
static void handle_ts_jump(obs_source_t *source, uint64_t expected, uint64_t ts, uint64_t diff, uint64_t os_time)
{
	...
	//PRISM/FanZirong/20231227/noissue/add log
	source->handle_ts_jump_count += 1;
	if (source->handle_ts_jump_count <= 20 || source->handle_ts_jump_count % 500 == 0) {
		blog(LOG_INFO, "source '%s', handle_ts_jump diff = %llu, reset_audio_timing timing_adjust = %llu",
		     source->context.name, diff, source->timing_adjust);
	}
}
...
static void source_output_audio_data(obs_source_t *source, const struct audio_data *data)
{
	...
	//PRISM/FanZirong/20231227/noissue/add log
	if (uint64_diff(os_time, source->source_update_time) < 2000000000) {
		source->reset_audio_frames = 0;
		source->ignore_audio_frames = 0;
	} else {
		uint64_t get_duration = conv_frames_to_time(sample_rate, source->get_audio_frames);
		if (get_duration == 0 || get_duration > 60000000000 * 2) {
			...
			blogex(false, LOG_INFO, fields, 2, "[%p][%s]capture audio pts %llu", source,
		...
		if (get_duration > 60000000000 * 2) {
			...
			if (drop_percentage > 0.05f) {
				...
				blogex(false, LOG_WARNING, fields, 1,
				       "[%s]drop audio %f, maybe audio noise, reset_all_audio = %llu, ignore_all_audio = %llu, get_all_audio = %llu",
				       source->context.name, drop_percentage, reset_duration, ignore_duration,
				       get_duration);
			}
	...
	source->get_audio_frames += in.frames;
	...
	if (source->next_audio_sys_ts_min == in.timestamp) {
		...
		if (diff < TS_SMOOTHING_THRESHOLD) {
			...
			//PRISM/FanZirong/20231227/noissue/add log
			source->handle_sys_jump_count += 1;
			if (source->handle_sys_jump_count <= 20 || source->handle_sys_jump_count % 500 == 0) {
				blog(LOG_INFO,
				     "source '%s', sys jump diff = %llu, reset_audio_timing, timing_adjust = %llu",
				     source->context.name, diff, source->timing_adjust);
			}
...
static inline void obs_source_main_render(obs_source_t *source)
{
	...
	//PRISM/Zengqin/20240528/none/add render source profile
	if (!source->source_render_profile) {
		source->source_render_profile = profile_store_name(obs_get_profiler_name_store(),
								   "render_source(%p, %s)", source, source->info.id);
	}
	...
	//PRISM/Zengqin/20240528/none/add render source profile
	if (source->source_render_profile)
		profile_end(source->source_render_profile);
...
void obs_source_show_preloaded_video(obs_source_t *source)
{
	...
	//PRISM/FanZirong/20231227/noissue/add log
	blog(LOG_INFO, "source '%s', obs_source_show_preloaded_video reset_audio_timing timing_adjust = %llu",
	     source->context.name, source->timing_adjust);
}
...
static void process_audio(obs_source_t *source, const struct obs_source_audio *audio)
{
	...
	if (source->resampler) {
		...
		//PRISM/FanZirong/20230912/2516/return when resample failed
		if (!audio_resampler_resample(source->resampler, output, &frames, &source->resample_offset, audio->data,
					      audio->frames)) {
			return;
		}
...
//PRISM/FanZirong/20231227/noissue/add log
void obs_source_notify_update(obs_source_t *source, uint64_t update_time)
{
	source->source_update_time = update_time;
}
...
void obs_source_skip_video_filter(obs_source_t *filter)
{
	...
	//PRISM/ZengQin/20240204/#4298/for nelo crash
	if (!parent || !target)
		return;
...
static float get_source_volume(obs_source_t *source, uint64_t os_time)
{
	...
	bool muted = !source->enabled || source->muted || (source->push_to_mute_enabled && push_to_mute_active) ||
		     (source->push_to_talk_enabled && !push_to_talk_active) || pls_get_all_mute();
