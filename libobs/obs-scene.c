...
//PRISM/Zhongling/20240109/#2091/start: Render with default blending type and method when dual output
static inline enum obs_blending_method pls_item_get_blend_method(const struct obs_scene_item *item)
{
	if (!item || !item->source) {
		return OBS_BLEND_METHOD_DEFAULT;
	}

	if (0 == strcmp(item->source->info.id, "group") && pls_is_dual_output_on()) {
		return OBS_BLEND_METHOD_DEFAULT;
	} else {
		return item->blend_method;
	}
}

static inline enum obs_blending_type pls_item_get_blend_type(const struct obs_scene_item *item)
{
	if (!item || !item->source) {
	...
	if (0 == strcmp(item->source->info.id, "group") && pls_is_dual_output_on()) {
		...
		return item->blend_type;
	}
}
//PRISM/Zhongling/20240109/#2091/end
...
static void *scene_create(obs_data_t *settings, struct obs_source *source)
{
	...
	//PRISM/Xiewei/20241111/PRISM_PC-1448/dual output
	if (settings)
		scene->is_vertical = obs_data_get_bool(settings, ID_IS_VERTICAL);
...
static bool is_items_normal(struct obs_scene *scene, const char *from_func)
{
	if (!pls_is_dev_mode() && !pls_is_local_log()) {
		// won't run this check in release mode
		return true;
	}

	bool is_normal = true;
	struct obs_scene_item *wrong_item = NULL;
	...
	if (!warned && wrong_item) {
		warned = true;

		blog(LOG_ERROR, "from %s, Scene %p has wrong items list, item=%p source=%p", from_func, scene,
		     wrong_item, wrong_item->source);
	}
...
//PRISM/wangshaohui/20250108/add logs to trace item->parent
#define detach_sceneitem(x) detach_sceneitem2(x, __FUNCTION__)
static inline void detach_sceneitem2(struct obs_scene_item *item, const char *from_func)
{
	//PRISM/Xiewei/20250108/add logs to trace item->parent
	blog(LOG_INFO, "%s-%p oldParent=%p Bind new parent with NULL from %s", __FUNCTION__, (void *)item, item->parent,
	     from_func);

	if (item->prev)
		item->prev->next = item->next;
	// check nullptr for item parent:https://jira.navercorp.com/browse/PRISM_PC_NELO-235
	else if (item->parent)
...
//PRISM/wangshaohui/20250108/add logs to trace item->parent
#define attach_sceneitem(x, y, z) attach_sceneitem2(x, y, z, __FUNCTION__)
static inline void attach_sceneitem2(struct obs_scene *parent, struct obs_scene_item *item, struct obs_scene_item *prev,
				     const char *from_func)
{
	//PRISM/Xiewei/20250108/add logs to trace item->parent
	blog(LOG_INFO, "%s-%p oldParent=%p Bind new parent(%p) from %s", __FUNCTION__, (void *)item, item->parent,
	     (void *)parent, from_func);
...
//PRISM/wangshaohui/20250108/PRISM_PC-2047/wrong sceneitem value
#include "graphics/math-defs.h"
static void check_group_scene_item(const char *func, obs_sceneitem_t *item)
{
	if (!item || !item->source || !item->source->info.id || strcmp(item->source->info.id, "scene") != 0) {
		return;
	}
	...
	// here this is a scene source for group reference

	if (!close_float(item->scale.x, 1.0, EPSILON) || !close_float(item->scale.y, 1.0, EPSILON) ||
	    !close_float(item->pos.x, 0.0, EPSILON) || !close_float(item->pos.x, 0.0, EPSILON)) {
		blog(LOG_WARNING, "[%s] wrong value for scene source. item=%p source=%p pos=(%f, %f) scale=(%f, %f)",
		     func, item, item->source, item->pos.x, item->pos.y, item->scale.x, item->scale.y);
		assert(false);
	}
}
...
{
	return pls_item_get_blend_type(item) == OBS_BLEND_NORMAL;
}
...
static inline bool item_texture_enabled(const struct obs_scene_item *item)
{
	return crop_enabled(&item->crop) || crop_enabled(&item->bounds_crop) || scale_filter_enabled(item) ||
	       (pls_item_get_blend_method(item) == OBS_BLEND_METHOD_SRGB_OFF) || !default_blending_enabled(item) ||
				...
				enum gs_color_space source_space)
{
	...
	gs_blend_function_separate(obs_blend_mode_params[pls_item_get_blend_type(item)].src_color,
				   obs_blend_mode_params[pls_item_get_blend_type(item)].dst_color,
				   obs_blend_mode_params[pls_item_get_blend_type(item)].src_alpha,
				   obs_blend_mode_params[pls_item_get_blend_type(item)].dst_alpha);
	gs_blend_op(obs_blend_mode_params[pls_item_get_blend_type(item)].op);
...
static inline void render_item(struct obs_scene_item *item)
{
	...
	const bool linear_srgb = !item->item_render || (pls_item_get_blend_method(item) != OBS_BLEND_METHOD_SRGB_OFF);
...
static void scene_video_render(void *data, gs_effect_t *effect)
{
	...
	bool is_vertical_rendering = pls_vertical_canvas_is_rendering();
	pls_set_dual_output_current_check_on();
	if (!is_vertical_rendering)
...
static void set_visibility(struct obs_scene_item *item, bool vis)
{
	...
	//PRISM/wangshaohui/20250108/none/add item logs
	if (item->visible != vis) {
		blog(LOG_INFO, "%s modify item %p with visible=%d", __FUNCTION__, item, vis);
	}
...
//PRISM/chenguoxi/20241219/PRISM_PC-1867/dual output
static struct obs_scene_item *scene_load_item(struct obs_scene *scene, obs_data_t *item_data, obs_source_t *item_source,
					      obs_sceneitem_t *insert_after)
{
	...
	//PRISM/chenguoxi/20241219/PRISM_PC-1867/dual output
	if (item_source) {
		source = obs_source_get_ref(item_source);
	} else {
	...
	//PRISM/chenguoxi/20241219/PRISM_PC-1867/dual output
	item = obs_scene_add_internal(scene, source, insert_after, obs_data_get_int(item_data, "id"));
	if (!item) {
		blog(LOG_WARNING,
	...
	//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
	blog(LOG_INFO, "%s: add sceneitem %p: vertical=%d, parent=%p", __FUNCTION__, item, item->is_vertical,
	     item->parent);

	//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
	if (item->is_vertical) {
		if (!obs_hotkey_pair_set_vertical(item->toggle_visibility)) {
			blog(LOG_INFO, "%s: sceneitem %p set vertical failed!", __FUNCTION__, item);
		}
	}
...
static void scene_load(void *data, obs_data_t *settings)
{
	...
	for (i = 0; i < count; i++) {
		...
		//PRISM/chenguoxi/20241219/PRISM_PC-1867/dual output
		scene_load_item(scene, item_data, NULL, NULL);
...
static void scene_save_item(obs_data_array_t *array, struct obs_scene_item *item, struct obs_scene_item *backup_group)
{
	...
	if (item->is_group) {
		...
		while (group_item) {
			//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
			if (is_save_bypass_vertical && group_item->is_vertical) {
	...
	//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
	{
		obs_data_set_int(item_data, "hotkey_pair_id", item->toggle_visibility);
		obs_hotkey_pair_t *pair = obs_hotkey_get_pair_by_id(item->toggle_visibility);
		if (pair) {
			obs_data_set_int(item_data, "hotkey_id_0", pair->id[0]);
			obs_data_set_int(item_data, "hotkey_id_1", pair->id[1]);
		}
	}
...
static void apply_scene_item_audio_actions(struct obs_scene_item *item, float *buf, uint64_t ts, size_t sample_rate)
{
	...
	for (size_t i = 0; i < item->audio_actions.num; i++) {
		...
		//PRISM/wangshaohui/20250108/none/add item logs
		if (item->visible != action.visible) {
			blog(LOG_INFO, "%s modify item %p with visible=%d", __FUNCTION__, item, action.visible);
		}
...
obs_scene_t *obs_scene_duplicate(obs_scene_t *scene, const char *name, enum obs_scene_duplicate_type type)
{
	...
	//PRISM/Zengqin/20230201/#none/for DrawPen feature
	pls_duplicate_scene_canvas(new_scene, scene);
	...
	for (size_t i = 0; i < items.num; i++) {
		...
		if (source) {
			...
			if (item->is_vertical) {
				new_item = pls_vertical_scene_add(new_scene, source, NULL, item->private_settings);
			} else {
...
static void init_hotkeys(obs_scene_t *scene, obs_sceneitem_t *item, const char *name)
{
	...
	//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
	if (!obs_hotkey_pair_set_is_sceneitem(item->toggle_visibility)) {
		blog(LOG_ERROR, "%s: sceneitem %p set is_sceneitem failed!", __FUNCTION__, item);
	}
					       ...
					       int64_t id)
{
	...
	//PRISM/chenguoxi/20241219/PRISM_PC-1867/dual output
	item->is_vertical = is_add_vertical_scene_item;
	...
	//PRISM/wangshaohui/20250108/none/add item logs
	blog(LOG_INFO, "%s modify item %p with visible=%d", __FUNCTION__, item, item->visible);
	...
	blog(LOG_INFO, "%s is called for sceneitem %p. source(%p) into scene(%p)", __FUNCTION__, item, source, scene);
	...
	do {
		if (scene->source->context.private) {
			break;
		} else if (pls_is_vertical_scene2(obs_sceneitem_get_source(item))) {
			break;
		} else if (pls_is_vertical_scene(scene)) {
			break;
		} else {
			init_hotkeys(scene, item, obs_source_get_name(source));
		}
...
//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
obs_sceneitem_t *obs_scene_add_vertical(obs_scene_t *scene, obs_source_t *source, obs_sceneitem_t *insert_after,
					obs_data_t *settings)
{
	is_add_vertical_scene_item = true;
	obs_sceneitem_t *item = obs_scene_add_internal(scene, source, insert_after, 0);
	is_add_vertical_scene_item = false;
...
static void obs_sceneitem_destroy(obs_sceneitem_t *item)
{
	if (item) {
		//PRISM/wangshaohui/20230720/none/add logs
		blog(LOG_INFO, "%s is started for sceneitem %p", __FUNCTION__, item);
		...
		//PRISM/wangshaohui/20230720/none/add logs
		blog(LOG_INFO, "%s is completed for sceneitem %p", __FUNCTION__, item);
	}
}
...
{
	//PRISM/wangshaohui/20230724/#2093/check source alive
	if (item && !pls_is_alive(item)) {
		blog(LOG_WARNING, "%s you are using destroied sceneitem (%p), this may cause a crash", __FUNCTION__,
		     item);
		assert(false);
		return;
	}
...
void obs_sceneitem_remove(obs_sceneitem_t *item)
{
	...
	//PRISM/wangshaohui/20230720/none/add logs
	blog(LOG_INFO, "%s is called for sceneitem %p, alive:%s", __FUNCTION__, item,
	     pls_is_alive(item) ? "yes" : "no");

	scene = item->parent;

	//PRISM/Liuying/20231116/#2445/avoid call obs_sceneitem_remove repeatedly crash
	if (!scene) {
		return;
	}
...
void sceneitem_restore(obs_data_t *data, void *vp)
{
	...
	//PRISM/chenguoxi/20241219/PRISM_PC-1867/dual output
	scene_load_item(scene, data, NULL, NULL);
}
...
{
	//PRISM/Xiewei/20240204/#4338/check null parent pointer
	if (!parent) {
		assert(false);
		//blog(LOG_INFO, "%s: Null parent pointer, command: %s",
...
void obs_sceneitem_select(obs_sceneitem_t *item, bool select)
{
	...
	if (0 == strcmp(item->source->info.id, "scene")) {
		if (pls_is_dual_output_on() && pls_is_vertical_scene(item->source->context.data)) {
			select = false;
		}
	}
...
void obs_sceneitem_set_pos(obs_sceneitem_t *item, const struct vec2 *pos)
{
	if (item) {
		...
		//PRISM/wangshaohui/20250108/PRISM_PC-2047/wrong sceneitem value
		check_group_scene_item(__FUNCTION__, item);
	}
}
...
void obs_sceneitem_set_order(obs_sceneitem_t *item, enum obs_order_movement movement)
{
	...
	//PRISM/chenguoxi/20241219/PRISM_PC-1861/dual output
	struct obs_scene_item *next2 = NULL;
	struct obs_scene_item *prev2 = NULL;
	struct obs_scene_item *tmp = NULL;

	tmp = item->next;
	while (tmp != NULL) {
		if (item->is_vertical == tmp->is_vertical) {
			next2 = tmp;
			break;
		}
		tmp = tmp->next;
	...
	if (movement == OBS_ORDER_MOVE_DOWN) {
		...
		attach_sceneitem(scene, item, prev2 ? prev2->prev : NULL);

	} else if (movement == OBS_ORDER_MOVE_UP) {
		//PRISM/chenguoxi/20241219/PRISM_PC-1861/dual output
		//attach_sceneitem(scene, item, next ? next : prev);
		attach_sceneitem(scene, item, next2 ? next2 : prev);
...
static bool group_item_transition(obs_scene_t *scene, obs_sceneitem_t *item, void *param)
{
	if (!param || !item)
		return true;
	//PRISM/Zhongling/20250106/PRISM_PC-1992/dual output
	if (0 == strcmp(item->source->info.id, "group") && pls_is_dual_output_on()) {
		return true;
	}
...
bool obs_sceneitem_set_visible(obs_sceneitem_t *item, bool visible)
{
	...
	//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
	if (item->parent->is_vertical) {
		if (item->parent->real_scene) {
			signal_parent(item->parent->real_scene, "item_visible", &cd);
		} else {
			blog(LOG_WARNING, "%s: sceneitem %p: scene %p does not bind real scene!", __FUNCTION__, item,
			     item->parent);
...
bool obs_scene_reorder_items(obs_scene_t *scene, obs_sceneitem_t *const *item_order, size_t item_order_size)
{
	...
	for (size_t i = 0; i < item_order_size; i++) {
		blog(LOG_INFO, "%s reorder items for scene=%p, %lu/%lu item=%p (parent=%p)", __FUNCTION__, scene, i + 1,
		     item_order_size, item_order[i], item_order[i] ? item_order[i]->parent : 0);
	}
			      ...
			      bool vertical)
{
	...
	while (item) {
		//PRISM/Xiewei/20241111/PRISM_PC-1448/dual output
		if (vertical != pls_is_vertical_sceneitem(item)) {
			item = item->next;
			continue;
		}
...
static void resize_scene(obs_scene_t *scene)
{
	...
	struct vec2 maxv;
	struct vec2 scale;
	resize_scene_base(scene, &minv, &maxv, &scale, false);
}
...
{
	//PRISM/Xiewei/20241111/PRISM_PC-1448/dual output
	if (0 == strcmp(group->source->info.id, "group") && pls_is_dual_output_on())
		return;
...
obs_sceneitem_t *obs_scene_insert_group(obs_scene_t *scene, const char *name, obs_sceneitem_t **items, size_t count)
{
	...
	for (size_t i = 0; i < count; i++) {
		...
		//PRISM/Xiewei/20250108/add logs to trace item->parent
		blog(LOG_INFO, "%s-%p oldParent=%p Bind new parent(%p)", __FUNCTION__, (void *)items[idx],
		     items[idx]->parent, (void *)sub_scene);
...
void obs_sceneitem_group_ungroup(obs_sceneitem_t *item)
{
	...
	while (last) {
		...
		//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
		blog(LOG_INFO, "%s: add sceneitem %p: vertical=%d, parent=%p", __FUNCTION__, dst, dst->is_vertical,
		     dst->parent);

		//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
		if (dst->is_vertical) {
			if (!obs_hotkey_pair_set_vertical(dst->toggle_visibility)) {
				blog(LOG_INFO, "%s: sceneitem %p set vertical failed!", __FUNCTION__, dst);
			}
		}
...
//PRISM/chenguoxi/20250219/PRISM_PC-2342/dual output
void obs_sceneitem_group_add_item_ex(obs_sceneitem_t *group, obs_sceneitem_t *item, obs_sceneitem_t *insert_after)
{
	...
	if (item->parent != scene) {
		//PRISM/wangshaohui/20250108/none/add item logs
		blog(LOG_WARNING, "%s wrong parent. try to move item=%p (parent=%p) into group=%p (parent=%p)",
		     __FUNCTION__, item, item ? item->parent : 0, group, group ? group->parent : 0);
	...
	detach_sceneitem(item);
	attach_sceneitem(groupscene, item, insert_after);
...
{
	obs_sceneitem_group_add_item_ex(group, item, NULL);
}

//PRISM/chenguoxi/20250219/PRISM_PC-2342/dual output
void pls_obs_sceneitem_group_add_item(obs_sceneitem_t *group, obs_sceneitem_t *item, obs_sceneitem_t *insert_after)
{
	obs_sceneitem_group_add_item_ex(group, item, insert_after);
}
...
static void obs_sceneitem_move_hotkeys(obs_scene_t *parent, obs_sceneitem_t *item)
{
	...
	//PRISM/chenguoxi/20241219/PRISM_PC-1867/dual output
	if (pls_is_vertical_scene2(obs_sceneitem_get_source(item)) || pls_is_vertical_scene(parent))
		return;
...
//PRISM/chenguoxi/20241219/PRISM_PC-1867/dual output
void pls_obs_sceneitem_move_hotkeys(obs_scene_t *parent, obs_sceneitem_t *item)
{
	obs_sceneitem_move_hotkeys(parent, item);
}
...
bool obs_scene_reorder_items2(obs_scene_t *scene, struct obs_sceneitem_order_info *item_order, size_t item_order_size)
{
	...
	for (size_t i = 0; i < item_order_size; i++) {
		...
		blog(LOG_INFO, "%s reorder items for scene=%p, %lu/%lu group=%p (parent=%p) item=%p (parent=%p)",
		     __FUNCTION__, scene, i + 1, item_order_size, info->group, info->group ? info->group->parent : 0,
		     info->item, info->item ? info->item->parent : 0);
	}
	...
	for (size_t i = 0; i < item_order_size; i++) {
		...
		if (info->item->is_group) {
			...
			for (i++; i < item_order_size; i++) {
				...
				//PRISM/Xiewei/20250108/add logs to trace item->parent
				blog(LOG_INFO, "%s-%p oldParent=%p Bind new parent(%p)", __FUNCTION__, (void *)sub_item,
				     sub_item->parent, (void *)sub_scene);
		...
		//PRISM/Xiewei/20250108/add logs to trace item->parent
		blog(LOG_INFO, "%s-%p oldParent=%p Bind new parent(%p)", __FUNCTION__, (void *)item, item->parent,
		     (void *)scene);
...
//PRISM/Liuying/20230804/#2191/fixed deaklock when ungroup
void pls_sceneitem_group_ungroup(obs_sceneitem_t *item)
{
	if (!item || !item->is_group)
		return;
	...
	last = first;
	while (last) {
		da_push_back(items, &last);
		obs_sceneitem_addref(last);
		last = last->next;
	}
	full_unlock(subscene);
	full_unlock(scene);
	...
	for (size_t i = 0; i < items.num; i++) {
		obs_sceneitem_t *dst;
		remove_group_transform(item, items.array[i]);
		dst = obs_scene_add_internal(scene, items.array[i]->source, insert_after, 0);
		duplicate_item_data(dst, items.array[i], true, true);
		...
		apply_group_transform(items.array[i], item);
		insert_after = dst;
	}
	for (size_t i = 0; i < items.num; i++)
		obs_sceneitem_release(items.array[i]);
...
struct obs_video_info_v2 *obs_sceneitem_get_canvas(obs_sceneitem_t *item)
{
	...
	int index = item->is_vertical ? VERTIVAL_CANVAS_INDEX : LANDSCAPE_CANVAS_INDEX;
	return obs_get_canvas_by_index(index);
}

//PRISM/liuying/20250103/PRISM_PC-1944/get sceneitem by id and uuid
obs_sceneitem_t *pls_scene_find_sceneitem_by_id_uuid(obs_scene_t *scene, int64_t id, const char *uuid)
{
	struct obs_scene_item *item;
	...
	while (item) {
		if (item->id == id) {
			...
			const char *uuid_ = obs_data_get_string(settings, "uuid");
			if (uuid_ && 0 == strcmp(uuid_, uuid)) {
...
//PRISM/chenguoxi/20241219/PRISM_PC-1867/dual output
struct obs_scene_item *pls_scene_load_item(struct obs_scene *scene, obs_data_t *item_data, obs_source_t *item_source,
					   obs_sceneitem_t *insert_after)
{
	return scene_load_item(scene, item_data, item_source, insert_after);
}

//PRISM/chenguoxi/20241219/PRISM_PC-1867/dual output
void pls_scene_save_item(obs_data_t *item_data, struct obs_scene_item *item)
{
	if (item_data == NULL || item == NULL) {
		blog(LOG_ERROR, "%s item_data or item is NULL", __FUNCTION__);
		return;
	}

	if (item->is_group) {
		blog(LOG_ERROR, "%s want to save group: item=[%p]", __FUNCTION__, item);
		assert(false);
		return;
	}
	...
	if (obs_data_array_count(array) == 0) {
		obs_data_array_release(array);
		blog(LOG_ERROR, "%s get null array!", __FUNCTION__);
		return;
	}

	obs_data_t *data = obs_data_array_item(array, 0);
	obs_data_apply(item_data, data);
	obs_data_release(data);
	obs_data_array_release(array);
}
...
EXPORT void pls_get_sceneitem_hotkey_ids(struct obs_scene_item *item, PLS_SCENEITEM_HOTKEY_IDS *ids)
{
	...
	ids->hotkey_pair_id = item->toggle_visibility;
	obs_hotkey_pair_t *pair = obs_hotkey_get_pair_by_id(item->toggle_visibility);
	if (pair) {
		ids->hotkey_id_0 = pair->id[0];
		ids->hotkey_id_1 = pair->id[1];
	} else {
		ids->hotkey_id_0 = OBS_INVALID_HOTKEY_PAIR_ID;
		ids->hotkey_id_1 = OBS_INVALID_HOTKEY_PAIR_ID;
	}
}
