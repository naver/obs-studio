					       ...
					       int64_t id)
{
	...
	blog(LOG_INFO,
	     "%s is called for sceneitem %p. source(%p) into scene(%p)",
	     __FUNCTION__, item, source, scene);
...
static void obs_sceneitem_destroy(obs_sceneitem_t *item)
{
	if (item) {
		...
		//PRISM/wangshaohui/20230720/none/add logs
		blog(LOG_INFO, "%s is started for sceneitem %p", __FUNCTION__,
		     item);
		...
		//PRISM/wangshaohui/20230720/none/add logs
		blog(LOG_INFO, "%s is completed for sceneitem %p", __FUNCTION__,
		     item);
	}
}
...
void obs_sceneitem_addref(obs_sceneitem_t *item)
{
	...
	if (item && !pls_is_alive(item)) {
		blog(LOG_WARNING,
		     "%s you are using destroied sceneitem (%p), this may cause a crash",
		     __FUNCTION__, item);
		assert(false);
		return;
	}
...
void obs_sceneitem_remove(obs_sceneitem_t *item)
{
	...
	//PRISM/wangshaohui/20230720/none/add logs
	blog(LOG_INFO, "%s is called for sceneitem %p, alive:%s", __FUNCTION__,
	     item, pls_is_alive(item) ? "yes" : "no");
...
void pls_sceneitem_group_ungroup(obs_sceneitem_t *item)
{
	...
	for (size_t i = 0; i < items.num; i++) {
		...
		dst = obs_scene_add_internal(scene, items.array[i]->source,
					     insert_after, 0);
