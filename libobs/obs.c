...
static int obs_init_video_mix(struct obs_video_info *ovi, struct obs_core_video_mix *video)
{
	...
	//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
	if (num && obs->video.main_mix && !pls_is_vertical_ovi(video->ovi_v2)) {
	...
	if (video->gpu_conversion && !obs_init_gpu_conversion(video))
		goto fail; //PRISM/ZengQin/20250214/PRISM_PC-2308/Release the graphics lock
	...
	//PRISM/ZengQin/20250214/PRISM_PC-2308/Release the graphics lock
fail:
	gs_leave_context();
	return OBS_VIDEO_FAIL;
}

//PRISM/chenguoxi/20241104/PRISM_PC-1971/dual output
struct obs_core_video_mix *obs_create_video_mix(struct obs_video_info_v2 *ovi_v2)
{
	struct obs_core_video_mix *video = bzalloc(sizeof(struct obs_core_video_mix));

	//PRISM/chenguoxi/20241104/PRISM_PC-1971/dual output
	assert(ovi_v2 != NULL);
	assert(ovi_v2->ovi != NULL);
	video->ovi = ovi_v2->ovi;
	video->ovi_v2 = ovi_v2;

	if (obs_init_video_mix(ovi_v2->ovi, video) != OBS_VIDEO_SUCCESS) {
		bfree(video);
		video = NULL;
	}
	return video;
...
static int obs_init_video(bool start_graphics_thread)
{
	struct obs_core_video *video = &obs->video;
	...
	//PRISM/chenguoxi/20250318/PRISM_PC-2481/check if the thread starts
	...
	for (size_t i = 0, num = obs->video.canvases.num; i < num; i++) {
		...
		blog(LOG_INFO, "[CANVAS]: %s: ovi=%p, ovi_v2=%p, num=%zu, video=%p", __FUNCTION__, ovi->ovi, ovi, i, v);

		if (!v)
			return OBS_VIDEO_FAIL;
	}

	if (start_graphics_thread) {
		int errorcode;
#ifdef __APPLE__
		pthread_attr_t attr;
...
void obs_free_video_mix(struct obs_core_video_mix *video)
{
	...
	blog(LOG_INFO, "%s for obs_core_video_mix = %p", __FUNCTION__, video);

	//PRISM/Xiewei/20240702/#5772/main_mix should be set to NULL after video reset start
	if (obs->video.main_mix == video) {
...
static const char *obs_signals[] = {
	...
	"void analog_codec_notify(ptr codec, ptr encodeDecode, bool hw)",
...
const char *pls_get_version_string(void)
{
	return PLS_VERSION;
}
...
//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
int obs_set_video_info(struct obs_video_info_v2 *canvas, struct obs_video_info *updated, bool start_graphics_thread)
{
	...
	*(canvas->ovi) = *updated;
	...
	source_profiler_reset_video(updated);
	return obs_init_video(start_graphics_thread);
}

//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
int obs_remove_video_info(struct obs_video_info_v2 *ovi, bool start_graphics_thread)
{
	...
	for (size_t i = 0; i < num; i++) {
		if (obs->video.canvases.array[i] == ovi) {
			...
			if (i > LANDSCAPE_CANVAS_INDEX) {
				obs->video.dual_output_canvas_created = false;
			}
...
struct obs_video_info_v2 *obs_create_3dr_ovi(struct obs_video_info *in)
{
	...
	*(ovi_v2->ovi) = *in;
	...
	pthread_mutex_unlock(&obs->video.thirdpart_ovi_mutex);
	blog(LOG_INFO, "[VIDEO_CANVAS_OUTER] created ovi=%p, ovi_v2=%p, in=%p, index=[%zu]", ovi, ovi_v2, in,
	     obs->video.thirdpart_ovi_pool.num - 1);
...
struct obs_video_info_v2 *obs_find_ovi_v2_by_ovi(struct obs_video_info *ovi)
{
	...
	pthread_mutex_lock(&obs->video.thirdpart_ovi_mutex);
	for (size_t i = 0, num = obs->video.thirdpart_ovi_pool.num; i < num; i++) {
		struct obs_video_info_v2 *ovi_v2 = obs->video.thirdpart_ovi_pool.array[i];
		if (ovi_v2->ovi == ovi) {
			ret = ovi_v2;
			break;
		}
...
//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
static struct obs_video_info_v2 *build_ovi_v2()
{
	struct obs_video_info_v2 *ovi_v2 = bzalloc(sizeof(struct obs_video_info_v2));
	struct obs_video_info *ovi = bzalloc(sizeof(struct obs_video_info));
...
//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
static void ensure_canvas_num(size_t num)
{
	pthread_mutex_lock(&obs->video.canvases_mutex);
	size_t current_num = obs->video.canvases.num;
	for (size_t i = current_num; i < num; i++) {
		struct obs_video_info_v2 *ovi_v2 = build_ovi_v2();
		da_push_back(obs->video.canvases, &ovi_v2);
		blog(LOG_INFO, "[VIDEO_CANVAS] created ovi=%p, v2=%p, index=[%zu]", ovi_v2->ovi, ovi_v2, i);
	}
	pthread_mutex_unlock(&obs->video.canvases_mutex);
}

//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
struct obs_video_info_v2 *obs_create_landscape_video_info_v2()
{
	ensure_canvas_num(LANDSCAPE_CANVAS_INDEX + 1);
	return obs->video.canvases.array[LANDSCAPE_CANVAS_INDEX];
}

//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
struct obs_video_info_v2 *obs_create_vertical_video_info_v2()
{
	ensure_canvas_num(VERTIVAL_CANVAS_INDEX + 1);
	return obs->video.canvases.array[VERTIVAL_CANVAS_INDEX];
}
...
int obs_reset_video(struct obs_video_info *ovi)
{
	...
	//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
	canvas = obs_create_landscape_video_info_v2();

	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s", ovi, __FUNCTION__);

	return obs_set_video_info(canvas, ovi, true);
}
...
bool obs_get_video_info_by_index(size_t index, struct obs_video_info *ovi)
{
	...
	struct obs_video_info_v2 *ovi_v2 = obs_get_canvas_by_index(index);
	if (ovi_v2 == NULL) {
		return false;
	}
...
{
	if (is_dual_output_current_check_on && !obs_in_task_thread(OBS_TASK_GRAPHICS)) {
		assert(false);
...
{
	//PRISM/Xiewei/20240702/#5772/main_mix should be set to NULL after video reset
...
void obs_render_main_texture_internal(enum gs_blend_type src_c, enum gs_blend_type dest_c, enum gs_blend_type src_a,
...
obs_data_array_t *obs_save_sources_filtered(obs_save_source_filter_cb cb, void *data_)
{
	...
	while (source) {
		//PRISM/chenguoxi/20241104/PRISM_PC-1452/dual output
		if (is_save_bypass_vertical && pls_is_vertical_scene_or_group(source)) {
			source = (obs_source_t *)source->context.hh.next;
			continue;
		}
		     ...
		     void (*callback)(void *param, struct video_data *frame), void *param)
{
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Enter] video=%p", v, __FUNCTION__, video);

	if (video) {
		os_atomic_inc_long(&video->raw_active);

		//PRISM/WuLongyue/20231122/#2212/add logs
		blog(LOG_INFO, "%p-%s: raw_active=%ld", v, __FUNCTION__, os_atomic_load_long(&video->raw_active));
	}
...
void stop_raw_video(video_t *v, void (*callback)(void *param, struct video_data *frame), void *param)
{
	...
	if (video) {
		...
		//PRISM/WuLongyue/20231122/#2212/add logs
		bool normal = os_atomic_load_long(&video->raw_active) >= 0;
		blog(normal ? LOG_INFO : LOG_ERROR, "%p-%s: raw_active=%ld", v, __FUNCTION__,
		     os_atomic_load_long(&video->raw_active));
	}
...
bool start_gpu_encode(obs_encoder_t *encoder)
{
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Enter] video=%p", encoder, __FUNCTION__, video);
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Exit] success=%d, gpu_encoder_active=%ld", encoder, __FUNCTION__, success,
	     os_atomic_load_long(&video->gpu_encoder_active));
...
void stop_gpu_encode(obs_encoder_t *encoder)
{
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	bool normal = os_atomic_load_long(&video->gpu_encoder_active) >= 0;
	blog(normal ? LOG_INFO : LOG_ERROR, "%p-%s: [Exit] call_free=%d, gpu_encoder_active=%ld", encoder, __FUNCTION__,
	     call_free, os_atomic_load_long(&video->gpu_encoder_active));
}

bool obs_video_active(void)
{
	if (os_atomic_load_long(&obs->video.additional_active) > 0) {
		return true;
	}
...
void obs_video_inc_active(void)
{
	os_atomic_inc_long(&obs->video.additional_active);

	blog(LOG_INFO, "%p-%s: additional_active=%ld", &obs->video, __FUNCTION__,
	     os_atomic_load_long(&obs->video.additional_active));
}

void obs_video_dec_active(void)
{
	os_atomic_dec_long(&obs->video.additional_active);
