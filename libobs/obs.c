		     ...
		     void *param)
{
	...
	if (video) {
		...
		//PRISM/WuLongyue/20231122/#2212/add logs
		blog(LOG_INFO, "%p-%s: raw_active=%ld", v, __FUNCTION__,
		    ...
		    void *param)
{
	...
	if (video) {
		...
		blog(normal ? LOG_INFO : LOG_ERROR, "%p-%s: raw_active=%ld", v,
		     __FUNCTION__, os_atomic_load_long(&video->raw_active));
	}
...
bool start_gpu_encode(obs_encoder_t *encoder)
{
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Exit] success=%d, gpu_encoder_active=%ld", encoder, __FUNCTION__,
	     success, os_atomic_load_long(&video->gpu_encoder_active));
...
void stop_gpu_encode(obs_encoder_t *encoder)
{
	...
	blog(normal ? LOG_INFO : LOG_ERROR,
	     "%p-%s: [Exit] call_free=%d, gpu_encoder_active=%ld", encoder,
	     __FUNCTION__, call_free,
...
void obs_video_inc_active(void)
{
	...
	blog(LOG_INFO, "%p-%s: additional_active=%ld", &obs->video,
	     __FUNCTION__, os_atomic_load_long(&obs->video.additional_active));
}
