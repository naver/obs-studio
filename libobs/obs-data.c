...
static json_t *obs_data_to_json(obs_data_t *data, bool with_defaults)
{
	...
	//PRISM/aiguanghua/20241219/PRISM_PC_NELO-138/add data empty
	if (!data) {
		return json;
	}
...
const char *pls_data_get_json(obs_data_t *data)
{
	if (!data)
		return NULL;
	...
	json_t *root = obs_data_to_json(data, false);
	data->json = json_dumps(root, JSON_COMPACT | JSON_SORT_KEYS);
	json_decref(root);
