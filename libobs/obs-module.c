...
static void load_all_callback(void *param, const struct obs_module_info2 *info)
{
	...
	if (!is_safe_module(info->name)) {
		blog(LOG_DEBUG, "Skipping module '%s', not on safe list",
		     info->name);
		return;
	}
	...
	if (pls_is_dll_already_loaded(file)) {
		blog(LOG_DEBUG, "Module file '%s' has been loaded, Ignore it.",
		     info->bin_path);
		blog(LOG_INFO, "Module file '%s' has been loaded, Ignore it.",
		     file);
		return;
	}
	...
	if (pls_is_plugin_in_black_list(file)) {
		blog(LOG_DEBUG, "Module file '%s' is in blacklist, Ignore it.",
		     info->bin_path);
		blog(LOG_INFO, "Module file '%s' is in blacklist, Ignore it.",
		     file);
		return;
	}
