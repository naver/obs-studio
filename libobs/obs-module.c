...
//PRISM/Xiewei/20230712/#1881 filter repeated obs plugins
...
static void load_all_callback(void *param, const struct obs_module_info2 *info)
{
	...
	if (!is_obs_plugin) {
		blog(LOG_DEBUG, "Skipping module '%s', not an OBS plugin", info->bin_path);
		return;
	}

	if (!is_safe_module(info->name)) {
		blog(LOG_DEBUG, "Skipping module '%s', not on safe list", info->name);
		return;
	}
	...
	//PRISM/Xiewei/20230712/#1881 filter repeated obs plugins
	file = strrchr(info->bin_path, '/');
	file = (!file) ? info->bin_path : (file + 1);

	if (pls_is_dll_already_loaded(file)) {
		blog(LOG_DEBUG, "Module file '%s' has been loaded, Ignore it.", info->bin_path);
		blog(LOG_INFO, "Module file '%s' has been loaded, Ignore it.", file);
		return;
	}

	//PRISM/Xiewei/20230712/#5169 ignore the plugins in blacklist
	if (pls_is_plugin_in_black_list(file)) {
		blog(LOG_DEBUG, "Module file '%s' is in blacklist, Ignore it.", info->bin_path);
		blog(LOG_INFO, "Module file '%s' is in blacklist, Ignore it.", file);
		return;
	}
...
#if _WIN32
	if (module) {
		bool internal_module = !!os_dlsym(module, "obs_is_internal_module");
		bool is_pls_module = os_is_pls_plugin(info->bin_path);
		if (!internal_module && !is_pls_module) {
	...
	//PRISM/Xiewei/20230712/#1881 filter repeated obs plugins
	if (module)
		pls_remember_dll_name(module->file);
