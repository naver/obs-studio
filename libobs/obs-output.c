				...
				obs_data_t *settings, obs_data_t *hotkey_data)
{
	...
	//PRISM/WuLongyue/20230721/none/add logs
	blog(LOG_INFO, "%s: output '%s' (%s) created: %p-%p", __FUNCTION__,
	     name, id, output, output->context.data);
	return output;
...
void obs_output_destroy(obs_output_t *output)
{
	if (output) {
		...
		//PRISM/WuLongyue/20230721/none/add logs
		blog(LOG_INFO, "%s: output '%s' to destroy: %p", __FUNCTION__,
		     output->context.name, output);
		...
		//PRISM/WuLongyue/20230721/none/add logs
		blog(LOG_INFO, "%s: output destroyed: %p", __FUNCTION__, output);
	}
}
