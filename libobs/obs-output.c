...
obs_output_t *obs_output_create(const char *id, const char *name, obs_data_t *settings, obs_data_t *hotkey_data)
{
	...
	//PRISM/chenguoxi/20241023/PRISM_PC_NELO-17/check output alive
	if (output != NULL)
		pls_add_alive(output);
	...
	//PRISM/cao.kewei/20241016/PRISM_PC-1296 PRISM_PC-1390
	pls_statistics_init(output);

	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: id=%s, name=%s, data=%p, video=%p", output, __FUNCTION__, id, name, output->context.data,
	     output->video);
...
//PRISM/WuLongyue/20231116/None/support NAVERShopping HEVC ------------ start
void obs_output_set_naver_shopping(obs_output_t *output, bool naver_shopping)
{
	output->is_naver_shopping = naver_shopping;
}
bool obs_output_get_naver_shopping(obs_output_t *output)
{
	return output->is_naver_shopping;
}
//PRISM/WuLongyue/20231116/None/support NAVERShopping HEVC ------------ end
...
void obs_output_destroy(obs_output_t *output)
{
	...
	if (output) {
		...
		//PRISM/WuLongyue/20231122/#2212/add logs
		blog(LOG_INFO, "%p-%s: position 1", output, __FUNCTION__);

		if (output->valid && active(output))
			obs_output_actual_stop(output, true, 0);

		//PRISM/WuLongyue/20231122/#2212/add logs
		blog(LOG_INFO, "%p-%s: position 2", output, __FUNCTION__);
		...
		//PRISM/WuLongyue/20231122/#2212/add logs
		blog(LOG_INFO, "%p-%s: position 3", output, __FUNCTION__);
		...
		//PRISM/WuLongyue/20231122/#2212/add logs
		blog(LOG_INFO, "%p-%s: position 4", output, __FUNCTION__);
		...
		//PRISM/WuLongyue/20231122/#2212/add logs
		blog(LOG_INFO, "%p-%s: position 5", output, __FUNCTION__);
		for (size_t i = 0; i < MAX_OUTPUT_VIDEO_ENCODERS; i++)
		...
		//PRISM/cao.kewei/20241016/PRISM_PC-1296 PRISM_PC-1390
		pls_statistics_destroy(output);

		//PRISM/chenguoxi/20241023/PRISM_PC_NELO-17/check output alive
		pls_remove_alive(output);

		bfree(output);
	}

	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Exit]", output, __FUNCTION__);
}
...
{
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: id=%s [Enter]", output, __FUNCTION__, output->info.id);

	//PRISM/wangshaohui/20250409/PRISM_PC-2599/checking video_t
	//reconnecting may call this function, so here need to check video
	if (!pls_check_output_video(output, __FUNCTION__)) {
		blog(LOG_WARNING, "%p-%s: id=%s using valid video and return false", output, __FUNCTION__,
		     output->info.id);
		return false;
	}
	...
	//PRISM/wangshaohui/20250115/none/add logs
	if (!success) {
		const char *error = obs_output_get_last_error(output);
		blog(LOG_INFO, "%p-%s: id=%s name='%s' error:%s", output, __FUNCTION__, output->info.id,
		     output->context.name, error ? error : "no last error");
	}
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: id=%s success=%d [Exit]", output, __FUNCTION__, output->info.id, success);
...
bool obs_output_start(obs_output_t *output)
{
	...
	blog(LOG_INFO, "%p-%s: id=[%s] name=[%s] [Enter]", output, __FUNCTION__, output->info.id, name ? name : "none");

	if (flag_service(output) &&
	    !(obs_service_can_try_to_connect(output->service) && obs_service_initialize(output->service, output))) {

		//PRISM/WuLongyue/20231122/#2212/add logs
		blog(LOG_INFO, "%p-%s: [Exit with failed for service] id=[%s] service logic failed", output,
	...
	if (output->delay_sec) {
		...
		//PRISM/WuLongyue/20231122/#2212/add logs
		blog(LOG_INFO, "%p-%s: [Exit with failed for start] id=[%s] obs_output_actual_start failed", output,
		     __FUNCTION__, output->info.id);
...
{
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Enter] force=%d, ts=%llu id=%s", output, __FUNCTION__, force, ts, output->info.id);
	...
	if (stopping(output) && !force) {
		//PRISM/WuLongyue/20231122/#2212/add logs
		blog(LOG_INFO, "%p-%s: [Exit] since it is stopping and not force stop", output, __FUNCTION__);
		return;
	}
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Step] position 1", output, __FUNCTION__);
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Step] position 2", output, __FUNCTION__);
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Step] position 3", output, __FUNCTION__);
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Step] position 4", output, __FUNCTION__);
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Exit] id=%s at the end", output, __FUNCTION__, output->info.id);
...
void obs_output_stop(obs_output_t *output)
{
	...
	if (!output->context.data) {
		//PRISM/WuLongyue/20231122/#2212/add logs
		blog(LOG_INFO, "%p-%s: [Exit] with context.data is empty", output, __FUNCTION__);
		return;
	}
	if (!active(output) && !reconnecting(output)) {
		//PRISM/WuLongyue/20231122/#2212/add logs
		blog(LOG_INFO, "%p-%s: [Exit] with not active and not reconnecting", output, __FUNCTION__);
		return;
	}
	if (reconnecting(output)) {
		obs_output_force_stop(output);
		//PRISM/WuLongyue/20231122/#2212/add logs
		blog(LOG_INFO, "%p-%s: [Exit] with being reconnecting", output, __FUNCTION__);
		return;
	}
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Exit] at the end", output, __FUNCTION__);
}
...
void obs_output_update(obs_output_t *output, obs_data_t *settings)
{
	...
	if (settings) {
		const char *json = obs_data_get_json(output->context.settings);
		const char *name = obs_output_get_name(output);
		blog(LOG_DEBUG, "%p - %s, output settings is set for %s (%s): \n%s\n", output, __FUNCTION__,
		     output->info.id ? output->info.id : "null", name ? name : "null", json ? json : "null");
	}
...
void obs_output_set_media(obs_output_t *output, video_t *video, audio_t *audio)
{
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: video=%p, audio=%p", output, __FUNCTION__, video, audio);
...
void obs_output_remove_encoder_internal(struct obs_output *output, struct obs_encoder *encoder)
{
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: encoder=%p", output, __FUNCTION__, encoder);
...
void obs_output_set_video_encoder2(obs_output_t *output, obs_encoder_t *encoder, size_t idx)
{
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: idx=%zu, encoder=%p", output, __FUNCTION__, idx, encoder);
...
static inline void check_received(struct obs_output *output, struct encoder_packet *out)
{
	if (out->type == OBS_ENCODER_VIDEO) {
		if (!output->received_video[out->track_idx]) {
			...
			//PRISM/wangshaohui/20250113/none/add logs
			const char *name = obs_output_get_name(output);
			blog(LOG_INFO, "%s: video is ready. output=%p id=%s name=%s videotrack=%lu", __FUNCTION__,
			     output, obs_output_get_id(output), name ? name : "null", out->track_idx);
		}
	} else {
		if (!output->received_audio) {
			...
			//PRISM/wangshaohui/20250113/none/add logs
			const char *name = obs_output_get_name(output);
			blog(LOG_INFO, "%s: audio is ready. output=%p id=%s name=%s", __FUNCTION__, output,
			     obs_output_get_id(output), name ? name : "null");
		}
	}
}
...
static inline void send_interleaved(struct obs_output *output)
{
	...
	//PRISM/cao.kewei/20240429/#5272/add logs
	bool first_video_packet = out.type == OBS_ENCODER_VIDEO && output->total_frames == 0;
	if (first_video_packet) {
		const char *name = obs_output_get_name(output);
		blog(LOG_INFO, "%p-%s: output received first video packet id=%s name=%s", output, __FUNCTION__,
		     obs_output_get_id(output), name ? name : "null");
	}
	if (stopping(output) && out.dts % 1000 == 0) {
		const char *name = obs_output_get_name(output);
		blog(LOG_INFO, "%p-%s: stopping id=%s name=%s", output, __FUNCTION__, obs_output_get_id(output),
		     name ? name : "null");
	}
...
static void interleave_packets(void *data, struct encoder_packet *packet, struct encoder_packet_time *packet_time)
{
	...
	//PRISM/cao.kewei/20241016/PRISM_PC-1296
	if (packet->type == OBS_ENCODER_VIDEO) {
		pls_statistics_log_pts(output, (struct pls_statistics_item){.packet_pts = packet->pts,
	...
	if (packet->type == OBS_ENCODER_VIDEO && !output->received_video[packet->track_idx] && !packet->keyframe) {
		...
		if (stopping(output)) {
			if (packet->dts % 50 == 0) {
				blog(LOG_INFO, "%p-%s: stopping output, id=%s. First video frame is not a keyframe.",
				     output, __FUNCTION__, obs_output_get_id(output));
			}
		}
	...
	if (was_started) {
		...
		//PRISM/cao.kewei/20241212/PRISM_PC-1563/pts offset
		int64_t original_pts = packet->pts;
		int64_t adjusted_pts = out.pts;
		pls_statistics_apply_offset(output, original_pts, adjusted_pts);
	} else {
	...
	if (output->received_audio && received_video) {
		if (!was_started) {
			//PRISM/wangshaohui/20250113/none/add logs
			const char *name = obs_output_get_name(output);
			blog(LOG_INFO, "%s: both audio and video are ready. output=%p id=%s name=%s", __FUNCTION__,
			     output, obs_output_get_id(output), name ? name : "null");
		...
		//PRISM/cao.kewei/20240429/#5272/add logs
		if (stopping(output) && packet->dts % 50 == 0) {
			blog(LOG_INFO, "%p-%s: stopping output, id=%s. received_audio: %d received_video: %d", output,
			     __FUNCTION__, obs_output_get_id(output), output->received_audio, received_video);
		}
	}
...
{
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: data=%p, video=%p, encoder=%p", output, __FUNCTION__, output->context.data,
	     output->video, output->video_encoders[0]);
...
void pls_add_active_output(const char *name);
static inline void signal_start(struct obs_output *output)
{
	do_output_signal(output, "start");

	//PRISM/WuLongyue/20240307/#2961/Save active output name
	pls_add_active_output(output->context.name);
}
...
static inline void signal_reconnect(struct obs_output *output)
{
	...
	blogex(false, LOG_INFO, fields, 3, "%p-%s: id=%s output_signal is called", output, __FUNCTION__,
	       output->info.id);
...
void pls_remove_active_output(const char *name);
static inline void signal_stop(struct obs_output *output)
{
	//PRISM/WuLongyue/20231122/#2212/add logs
	const char *fields[][2] = {{PTS_LOG_TYPE, PTS_TYPE_EVENT},
	...
	blogex(false, LOG_INFO, fields, 3,
	       "%p-%s: data=%p, video=%p, encoder=%p id=%s output_signal is called, stopCode=%d, lastError=%s", output,
	       __FUNCTION__, output->context.data, output->video, output->video_encoders[0], output->info.id,
	       output->stop_code, error ? error : "none");
	...
	//PRISM/WuLongyue/20240307/#2961/Save active output name
	pls_remove_active_output(output->context.name);
}
...
//PRISM/wangshaohui/20231214/none/add logs
void log_output_encoder(const char *func, obs_output_t *output, obs_encoder_t *encoder)
{
	const char *fields[][2] = {{PTS_LOG_TYPE, PTS_TYPE_EVENT},
				   {"output_id", output->info.id},
				   {"output_name", obs_output_get_name(output)}};

	obs_data_t *sts = obs_encoder_get_settings(encoder);
	const char *json = pls_data_get_json(sts);

	blogex(false, LOG_INFO, fields, 3,
	       "%s, successed to init encoder for output %p [%s]. \n"
	       "\t encoder = %p \n"
	       "\t type = %s \n"
	       "\t orig_id = %s \n"
	       "\t real_id = %s \n"
	       "\t encoder_settings = %s \n",
	       func, output, output->info.id ? output->info.id : "null", encoder,
	       encoder->orig_info.type == OBS_ENCODER_VIDEO ? "video_encoder" : "audio_encoder",
	       encoder->orig_info.id ? encoder->orig_info.id : "null", encoder->info.id ? encoder->info.id : "null",
	       json ? json : "null");
}
...
static inline bool initialize_audio_encoders(obs_output_t *output)
{
	for (size_t i = 0; i < MAX_OUTPUT_AUDIO_ENCODERS; i++) {
		...
		//PRISM/wangshaohui/20231214/none/add logs
		if (audio)
			log_output_encoder(__FUNCTION__, output, audio);
	}
...
static inline bool initialize_video_encoders(obs_output_t *output)
{
	for (size_t i = 0; i < MAX_OUTPUT_VIDEO_ENCODERS; i++) {
		...
		//PRISM/wangshaohui/20231214/none/add logs
		if (video)
			log_output_encoder(__FUNCTION__, output, video);
	}
...
bool obs_output_begin_data_capture(obs_output_t *output, uint32_t flags)
{
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: data=%p, video=%p, encoder=%p, falgs=%u", output, __FUNCTION__, output->context.data,
	     output->video, output->video_encoders[0], flags);
	...
	for (size_t i = 0; i < MAX_OUTPUT_VIDEO_ENCODERS; i++) {
		...
		if (encoder) {
			os_atomic_set_bool(&encoder->force_keyframe, true);
			os_atomic_set_long(&encoder->keyframe_cnt, 0);
		}
	}
	//PRISM/wangshaohui/20240628/none/log keyframe
	for (size_t i = 0; i < MAX_OUTPUT_AUDIO_ENCODERS; i++) {
		obs_encoder_t *encoder = output->audio_encoders[i];
		if (encoder) {
			os_atomic_set_long(&encoder->keyframe_cnt, 0);
		}
	}
	//PRISM/wangshaohui/20250311/2446/for network time
	pls_output_reset_send_time(output);

	if (flag_service(output))
		obs_service_activate(output->service);

	//PRISM/wangshaohui/20241206/none/add logs
	if (flag_encoded(output)) {
		const char *fields[][2] = {{PTS_LOG_TYPE, PTS_TYPE_EVENT},
					   {"output_id", output->info.id},
					   {"output_name", obs_output_get_name(output)}};

		video_t *v = (video_t *)output->video_encoders[0]->media;
		const struct video_output_info *voi = video_output_get_info(v);

		blogex(false, LOG_INFO, fields, 3,
		       "%p id=%s output video settings: \n"
		       "\t resolution: %dx%d \n"
		       "\t format: %s \n"
		       "\t fps: %d/%d \n"
		       "\t colorspace: %s \n"
		       "\t range: %s \n",
		       output, output->info.id, voi->width, voi->height, get_video_format_name(voi->format),
		       voi->fps_num, voi->fps_den, get_video_colorspace_name(voi->colorspace),
...
{
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Enter] callback=%p, data=%p, video=%p, encoder=%p", output, __FUNCTION__,
	     encoded_callback, output->context.data, output->video, output->video_encoders[0]);
	...
	blog(LOG_INFO, "%p-%s: [Exit] callback=%p", output, __FUNCTION__, encoded_callback);
}
...
static void *end_data_capture_thread(void *data)
{
	...
	//PRISM/wangshaohui/20250401/#2394/dump lock list
	os_set_thread_name("end_data_capture_thread");

	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Enter] data=%p, video=%p, encoder=%p", output, __FUNCTION__, output->context.data,
	     output->video, output->video_encoders[0]);
...
static void *reconnect_thread(void *param)
{
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Enter] data=%p, video=%p, encoder=%p id=%s name='%s' retryCount:%d interval=%u(ms)",
	     output, __FUNCTION__, output->context.data, output->video, output->video_encoders[0], output->info.id,
	     output->context.name, output->reconnect_retries, output->reconnect_retry_cur_msec);
	...
	if (os_event_timedwait(output->reconnect_stop_event, output->reconnect_retry_cur_msec) == ETIMEDOUT) {
		//PRISM/wangshaohui/20250115/none/add logs
		blog(LOG_INFO, "%p-%s: id=%s name='%s' retryCount:%d >> Try retry", output, __FUNCTION__,
		     output->info.id, output->context.name, output->reconnect_retries);

		obs_output_actual_start(output);

	} else { //PRISM/wangshaohui/20250115/none/add logs
		blog(LOG_INFO, "%p-%s: id=%s name='%s' retryCount:%d >> Cancel retry", output, __FUNCTION__,
		     output->info.id, output->context.name, output->reconnect_retries);
	}
...
void obs_output_signal_stop(obs_output_t *output, int code)
{
	...
	//PRISM/WuLongyue/20231122/#2212/add logs
	blog(LOG_INFO, "%p-%s: [Exit] id:%s", output, __FUNCTION__, output->info.id);
}
...
void obs_output_release(obs_output_t *output)
{
	...
	//PRISM/chenguoxi/20241023/PRISM_PC_NELO-17/check output alive
	if (!pls_is_alive(output)) {
		blog(LOG_WARNING, "%s you are using destroyed output (%p), this may cause a crash", __FUNCTION__,
		     output);
		assert(false);
		return;
	}
...
void obs_output_set_last_error(obs_output_t *output, const char *message)
{
	...
	if (message) {
		blog(LOG_WARNING, "%s output=%p id=%s '%s' error: %s", __FUNCTION__, output, output->info.id,
		     output->context.name, message);
	}
}
