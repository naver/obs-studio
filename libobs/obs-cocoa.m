...
char *find_libobs_data_file(const char *file)
{
    ...
    //PRISM/AiGuanghua/20241225/PRISM_PC_NELO-143/frameworkBundle is nil
    if (!frameworkBundle) {
        NSString *bundleFrameworkPath = [[[NSBundle mainBundle] bundlePath] stringByAppendingString:@"/Contents/Frameworks/libobs.framework"];
	blog(LOG_INFO, "get obs data file path using load framework path, file is %s", file);
	frameworkBundle = [NSBundle bundleWithPath:bundleFrameworkPath];
	if (!frameworkBundle) {
	   assert(false);
	}
    }
