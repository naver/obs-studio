...
CircleBufferIPC::CircleBufferIPC(const char *queueName, int extendSize, int itemHdrSz,
				 int itemCount, int itemSampleSize)
	  ...
	  mutexItems(new HANDLE[itemCount])
{
	...
	headerSizeAlign = ALIGN_NUM(extendSize + sizeof(QueueInfo), 64);
	itemSizeAlign = ALIGN_NUM(itemHdrSz + itemSampleSize, 64);

	bufUpdateEvent = CHandleWrapper::GetEvent(
		GenerateName(FILEMAP_BUFFER_CHANGED_EVENT, queueName, 0).c_str(), false);
	extUpdateEvent = CHandleWrapper::GetEvent(
		GenerateName(FILEMAP_EXTENSION_CHANGED_EVENT, queueName, 0).c_str(), false);
	ResetEvent(bufUpdateEvent);
	ResetEvent(extUpdateEvent);

	mutexTop = CHandleWrapper::GetMutex(
		GenerateName(FILEMAP_BUFFER_MUTEX_TOP, queueName, 0).c_str());
	for (int i = 0; i < itemCount; ++i) {
		mutexItems.get()[i] = CHandleWrapper::GetMutex(
			GenerateName(FILEMAP_BUFFER_MUTEX_ITEM, queueName, i).c_str());
	}
}
...
int CircleBufferIPC::WriteItemInner(void **hdr, int hdrWriteSize, void **data, int dataWriteSize,
				    bool keepLockItem)
{
...
int CircleBufferIPC::ReadItemInner(void **hdr, int hdrReadSize, void **buf, int dataReadSize,
				   bool keepLockItem)
{
...
void CircleBufferIPC::FormatMapView()
{
	...
	itemArray = std::shared_ptr<ItemInfo>(new ItemInfo[itemTotalCount],
					      std::default_delete<ItemInfo[]>());
	assert(itemArray);
	if (itemArray) {
...
bool CircleBufferIPC::IsDataReady()
{
	...
	if (queueInfo->writenCount > itemTotalCount &&
