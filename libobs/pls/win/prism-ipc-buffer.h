...
protected:
	explicit CircleBufferIPC(const char *queueName, int extendSize, int itemHeaderSize,
...
private:
	int WriteItemInner(void **hdr, int hdrWriteSize, void **data, int dataWriteSize,
			   bool keepLockItem);
	int ReadItemInner(void **hdr, int hdrReadSize, void **buf, int dataReadSize,
			  bool keepLockItem);
