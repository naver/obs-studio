...
#define do_logex(kr, level, fields, field_count, format, ...)                      \
	blogex(kr, level, fields, field_count, "[audio monitoring: '%s'] " format, \
...
struct audio_monitor {
	...
	//PRISM/FanZirong/20230726/none/reduce log
	bool get_device_failed;
...
static bool audio_monitor_init_wasapi(struct audio_monitor *monitor)
{
	...
	//PRISM/FanZirong/20230726/none/reduce log
	if (FAILED(hr)) {
		if (!monitor->get_device_failed) {
			warn("%s: Failed to get device: %08lX", __FUNCTION__, hr);
			monitor->get_device_failed = true;
		}
		goto fail;
	} else {
		if (monitor->get_device_failed) {
			monitor->get_device_failed = false;
			info("%s: get device successed", __FUNCTION__);
		}
	}
	...
	//PRISM/FanZirong/20230809/#2220/reduce log
	if (FAILED(hr)) {
		if (!monitor->active_device_failed) {
			warn("%s: Failed to activate device: %08lX", __FUNCTION__, hr);
			monitor->active_device_failed = true;
		}
		goto fail;
	} else {
		if (monitor->active_device_failed) {
			monitor->active_device_failed = false;
			info("%s: Successed to activate device", __FUNCTION__);
		}
	}
	...
	//PRISM/FanZirong/20230809/#2220/reduce log
	if (FAILED(hr)) {
		if (!monitor->initialize_client_failed) {
			warn("%s: Failed to initialize: %08lX", __FUNCTION__, hr);
			monitor->initialize_client_failed = true;
		}
		goto fail;
	} else {
		if (monitor->initialize_client_failed) {
			monitor->initialize_client_failed = false;
			info("%s: Successed to to initialize client", __FUNCTION__);
		}
	}
...
static void on_audio_playback(void *param, obs_source_t *source, const struct audio_data *audio_data, bool muted)
{
	...
	//PRISM/FanZirong/20241217/PRISM_PC-1778/if media source audio has cover, ignore av-sync
	if (monitor->source_has_video && !decouple_audio &&
	    (pls_source_is_audio_has_cover(source) == MP_NOT_READY ||
	     pls_source_is_audio_has_cover(source) == MP_NO_COVER)) {
	...
	if (monitor->is_first_audio_playback) {
		...
		const char *fields[][2] = {{PTS_LOG_TYPE, PTS_TYPE_EVENT}, {"monitor", monitor_p}};
		do_logex(false, LOG_INFO, fields, 2, "[monitor: %p]first audio playback", monitor);
...
static inline void audio_monitor_free(struct audio_monitor *monitor)
{
	...
	snprintf(monitor_p, sizeof(monitor_p), "%p", monitor);
	const char *fields[][2] = {{PTS_LOG_TYPE, PTS_TYPE_EVENT}, {"monitor", monitor_p}};
	do_logex(false, LOG_INFO, fields, 2, "[monitor: %p]audio_monitor_free", monitor);
...
static bool audio_monitor_init(struct audio_monitor *monitor, obs_source_t *source)
{
	...
	snprintf(monitor_p, sizeof(monitor_p), "%p", monitor);
	const char *fields[][2] = {{PTS_LOG_TYPE, PTS_TYPE_EVENT}, {"monitor", monitor_p}};
	do_logex(false, LOG_INFO, fields, 2, "[monitor: %p]audio_monitor_init", monitor);
